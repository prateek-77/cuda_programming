...
__global__ void sum_reduction(const int* arr, int* res_arr) {
    ...
    // Loop with stride 1, and expand it by 2 every iteration until it reaches block size.
    for (int s = 1; s < blockDim.x; s <<= 1) {
        // Only some threads do the addition.
        // Disadvantages - leads to warp divergence:
        // Only some threads in warp are active, while others are idle - leading to sequential nature
